...
Uses the vendored engine-agnostic PDK-runner wrappers in ``pdk_runner/``
(they drive the official IHP KLayout DRC/LVS decks headless via PDK_ROOT).
...
# vendored PDK runner provides run_drc/run_lvs. Loaded by path under a
# distinct module name (this file is also called signoff.py).
_OTA = os.path.join(HERE, "pdk_runner")
...
def _load_ota_signoff():
    ...
    spec = importlib.util.spec_from_file_location(
        "ota_signoff", os.path.join(_OTA, "ota_signoff.py"))
