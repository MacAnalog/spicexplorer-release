...
__all__ = [
    "PdkDevice", "Pdk", "IHP_SG13G2", "SKYWATER_SKY130", "GF180MCU", "ANALOGGYM_TSMC",
    "TSMC_N65", "PDKS", "get_pdk", "mos_flavor", "model_flavor", "split_flavor",
    "THRESHOLD_FLAVORS",
]
...
class Pdk:
    ...
    def resolve_model(
        ...
        * **threshold** (``lvt``/``ulvt``/``hvt``/``nvt``): substituted with the same voltage
          ...
          ``tsmc-n65`` whose default NMOS is literally ``nch_lvt``.
...
# Device-NAME map for the AnalogGym / ferrosim reference corpora in analog-db (TSMC-style
...
ANALOGGYM_TSMC = Pdk(
    ...
    width_per_finger=False,
    devices=(
        PdkDevice("nch_mac", DeviceType.MOS, MosPolarityType.NMOS),
        PdkDevice("pch_mac", DeviceType.MOS, MosPolarityType.PMOS),
        PdkDevice("nch_ulvt_mac", DeviceType.MOS, MosPolarityType.NMOS),
        PdkDevice("pch_ulvt_mac", DeviceType.MOS, MosPolarityType.PMOS),
        PdkDevice("nch_ehvt_mac", DeviceType.MOS, MosPolarityType.NMOS),
        ...
        PdkDevice("pch_18_mac", DeviceType.MOS, MosPolarityType.PMOS, flavor="hv"),
    ),
)

# --- TSMC N65 (the virtuoso-bridge / Spectre target) ------------------------------------
# Device-NAME table only — model names as they appear in netlists, no foundry model content
# (same posture as ANALOGGYM_TSMC). LVT first: it is the `model_for` default per polarity and
...
TSMC_N65 = Pdk(
    ...
    width_per_finger=False,
    devices=(
        PdkDevice("nch_lvt", DeviceType.MOS, MosPolarityType.NMOS),
        PdkDevice("pch_lvt", DeviceType.MOS, MosPolarityType.PMOS),
...
PDKS: dict[str, Pdk] = {
    ...
    ANALOGGYM_TSMC.name: ANALOGGYM_TSMC,
    TSMC_N65.name: TSMC_N65,
}
