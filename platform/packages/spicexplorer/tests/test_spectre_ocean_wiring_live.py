...
to the native-file path under test). Opt-in gating: bridge + `SPICEXPLORER_TSMC65_MODELS`
...
def test_live_full_loop_native_scs_injection_moves_the_metric(tmp_path: Path) -> None:
    ...
    deck = render_spectre_deck(
        deck_spec_from_ngspice(
            example, pdk="tsmc-n65", source_pdk="ihp-sg13g2",
...
project:
  ...
  outdir: out
  tech_spec: {{name: tsmc-n65, constraints: {{}}}}
  dut_params:
