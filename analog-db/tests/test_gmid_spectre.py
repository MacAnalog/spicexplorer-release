...
def test_config_reads_registry_block(cfg: SpectreGmidConfig) -> None:
    assert (cfg.nmos, cfg.pmos) == ("nch_lvt", "pch_lvt")
    assert cfg.lib_file == "tsmc_n65_models.scs"
...
def test_deck_is_nda_clean_and_well_formed(cfg: SpectreGmidConfig) -> None:
    ...
    assert 'tsmc_n65_models.scs" section=tt' in deck
    assert "crn65" not in deck.lower() and "/CMC" not in deck
    # both polarities, mirrored biases, nested sweep{dc}
    assert "XMN (vdn vgn 0 vbn) nch_lvt" in deck
    assert "XMP (vdp vgp 0 vbp) pch_lvt" in deck
...
def test_manifest_sidecar(tmp_path) -> None:
    ...
    assert path.name == "nch_lvt__tt.pkl" and path.parent.name == PDK
    man = json.loads(man_path.read_text())
    assert man["schema"] == "spicexplorer/gmid-lut@1"
    assert man["model"]["corner_lines"] == ['include "tsmc_n65_models.scs" section=tt']
...
def test_cli_dry_run_prints_deck(capsys) -> None:
    ...
    assert rc == 0
    assert "sweepvds sweep param=ds" in out and "nch_lvt" in out and "pch_lvt" in out
