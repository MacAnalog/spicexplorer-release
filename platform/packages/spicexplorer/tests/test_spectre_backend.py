...
def test_simresult_scalar_lookup_prefix_bare_and_op_point() -> None:
    # Key shapes mirror a real TSMC-65 run (P0 live, 2026-07-05): ac_/dc_ prefixed
...
# ---------------------------------------------------------------------------
# Shape copied from a real TSMC-65 `finalTimeOP.info` (P0 live run): per-model STRUCT
...
def test_apply_corner_emits_spectre_include_section_and_rails() -> None:
    ...
    corner = Corner(
        name="ss_hot",
        model_includes=[
            ModelInclude(lib_file="tsmc65.scs", section="ss"),
            ModelInclude(lib_file="tsmc65_rc.scs", section="rc_worst"),
        ],
        temp=85.0,
    ...
    assert staged["corner_includes"] == [
        'include "tsmc65.scs" section=ss',
        'include "tsmc65_rc.scs" section=rc_worst',
    ]
    ...
    sim.apply_corner(Corner(name="tt", model_includes=[ModelInclude("tsmc65.scs", "tt")]))
    assert sim.staged_params["corner"] == "tt"
    assert sim.staged_params["corner_includes"] == ['include "tsmc65.scs" section=tt']
...
def test_composed_mode_materializes_staged_params_per_run(tmp_path: Path) -> None:
    ...
    assert 'include "/opt/tsmc/models.scs" section=ss_lvt' in text  # corner selection
