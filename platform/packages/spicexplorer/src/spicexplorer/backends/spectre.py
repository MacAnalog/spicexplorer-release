...
# against real TSMC-65 psfascii (2026-07-05). Per-MOS op scalars (`M0:gm`) are merged in
    ...
    while i < len(lines):
        ...
        if m_open:
            ...
            while i < len(lines):
                ...
                # a trailing PROP annotation (real TSMC output: `"model" "nch_lvt.10"`)
                if inner.startswith(")"):
                    break
                try:
...
_SWEEP_EXT: dict[str, str] = {
    ...
    # live on TSMC-65, 2026-07-11). Spectre also leaves a `pnoise.pnoise.cache` sibling,
