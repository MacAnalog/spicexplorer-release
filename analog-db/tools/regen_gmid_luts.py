...
25 mV grid, and the tsmc-n65 tables come from a licensed kit (NDA). This script rebuilds the
...
Lanes (auto-selected per PDK by the registry ``gmid.engine`` / ``sim_engine`` marker):
  * **open PDKs** (sky130 / ihp-sg13g2 / gf180mcu) → native ngspice + ``$PDK_ROOT`` (per-L
    ...
    isn't available.
  * **tsmc-n65** → headless Spectre via the virtuoso-bridge (both polarities per pass, one
