...
S {}
E {}
T {amplifier/noise -- rrl-integrator-opamp standalone bench (amp_027_fan_rrl_ota)} 52.5 -1402.5 0 0 0.4 0.4 {}
...
only_toplevel=true
value="
* TESTBENCH noise -- DUT rrl-integrator-opamp (amp_027_fan_rrl_ota)
* template amplifier/noise_diff ; produces: vn_in (inoise_total; density vn_in over the band)
...
.control
  ...
  * spectra live in the noise1 plot; integrated totals live in the SEPARATE
  * noise2 plot (scalars). Broadcast the noise2 totals into noise1 as flat
  * vectors over frequency so ONE xschem graph can show density + integrated.
  setplot noise1
  let vn_in = sqrt(inoise_spectrum)
  let vn_in_nv = vn_in*1e9
  let vn_out_nv = sqrt(onoise_spectrum)*1e9
  let inoise_total_v = frequency*0 + noise2.inoise_total
  let onoise_total_v = frequency*0 + noise2.onoise_total
  write noise.raw vn_in vn_in_nv vn_out_nv inoise_spectrum onoise_spectrum inoise_total_v onoise_total_v
