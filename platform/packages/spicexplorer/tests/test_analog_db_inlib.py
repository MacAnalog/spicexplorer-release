...
def _have_marker() -> bool:
    try:
        return (
            analog_db_root() / "circuits" / _CIRCUIT / "pdk" / "tsmc-n65" / "sizing.yaml"
        ).is_file() and pdk_sim_engine("tsmc-n65") is not None
    except Exception:
        return False
...
def test_engine_capability_is_truthy_by_availability():
    assert bool(EngineCapability(_CIRCUIT, "tsmc-n65", "spectre", True, "ok"))
    assert not EngineCapability(_CIRCUIT, "tsmc-n65", "spectre", False, "no kit")
...
def test_circuitrun_out_signal_convention_per_engine():
    ...
    sp = CircuitRun(_CIRCUIT, "tsmc-n65", "spectre", "ac_open_loop", "tt", None, [])
...
def test_sim_engine_marker_routes_open_vs_closed():
    assert pdk_sim_engine("tsmc-n65") == "spectre"
...
def test_discovery_lists_circuit_pdks_and_analyses():
    ...
    assert set(circuit_pdks(_CIRCUIT)) >= {"ihp-sg13g2", "sky130", "tsmc-n65"}
...
def test_probe_engine_reports_engine_and_degrades_honestly():
    ...
    cap = probe_engine(_CIRCUIT, "tsmc-n65", model_lib_root="/nonexistent/model/root")
...
def test_run_circuit_raises_engine_unavailable_when_closed_lane_absent():
    ...
    with pytest.raises(EngineUnavailable, match="wrapper"):
        run_circuit(
            _CIRCUIT, "tsmc-n65", model_lib_root="/nonexistent/model/root",
...
def test_effective_supply_pins_the_two_truths():
    ...
    sup = effective_supply(_CIRCUIT, "tsmc-n65", "ac_open_loop")
    # amp_022's analyses bake VDD=1.5 (authored operating point) while the tsmc-n65
...
def test_evaluate_swaps_fft_recipes_for_pss_twins_on_spectre():
    ...
    run = CircuitRun("c", "tsmc-n65", "spectre", "iip3", "tt", _PssRes(), metrics)
