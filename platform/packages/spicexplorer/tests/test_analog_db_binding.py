...
_needs_db = pytest.mark.skipif(
    not _have_binding(),
    reason="analog-db amp_022 tsmc-n65 binding not checked out (set SPICEXPLORER_ANALOG_DB)",
)
...
def test_pdk_supply_is_core_rail():
    # the `vdd: 1.2` the tests hardcoded (CRN65 core rail), overriding the datasheet's 1.5 V default
...
def test_resolve_corner_is_generic_and_neutral():
    ...
    assert c.name == "tsmc-n65_tt"
    assert c.temp == 27.0
    # generic label + NEUTRAL wrapper — the kit-native `tt_lvt` never appears (operator wrapper maps it)
    assert [(i.lib_file, i.section) for i in c.model_includes] == [("tsmc_n65_models.scs", "tt")]
