# Drawings sizing & dual-engine validation report — ngspice/ihp + Spectre/tsmc-n65
...
> blocks, (4) xvport port of every drawing to Virtuoso/tsmcN65, (5) Spectre validation
...
- **Closed lane (Spectre, tsmc-n65, virtuoso-bridge)**: every drawing ported to real
  Virtuoso cellviews with `xvport` (device map sg13_lv_* → tsmcN65 nch_lvt/pch_lvt,
...
- **NDA**: kit model bytes are never read or committed; decks reference only the
  operator's local wrapper (`~/.spicexplorer/models/tsmc_n65_models.scs`, section tt).
...
## 5. Virtuoso / tsmc-n65 port — DONE, all oracles green (24 cells in `xvport_dev`)
...
## 6. Spectre (tsmc-n65) validation — DONE (drawn geometry, bias retune only)
...
the tsmc65 LVT single-stage gm/gds ceiling (journal-verified) — so 65 nm coming in
...
but **DC gain stays pinned at ~38 dB — the tsmc65 LVT gm/gds ceiling** (~17 dB below
