...
| `em.EmTech.from_yaml(path)` / `EmTech.builtin(name)` / `builtin_techs()` | **EM-lane tech config** — ALL PDK specifics (metal/via GDS layers, via stack, substrate-contact layer, port layer base, openEMS workflow + stackup location) live in YAML, not in the module; reference configs ship as package data under `src/spicexplorer_layout/techs/` (`EmTech.builtin("ihp-sg13g2")`) |
| `em.extract_net_gds(gds, nets, out, ports=, tech=)` | cut named nets out of a signed-off GDS (KLayout `LayoutToNetlist`, metal-only connectivity, labels name nets) into an EM-ready GDS + port rects; returns the manifest (`gds` extra) |
| `em.em_sparams(gds, ports, out_dir, tech=, sim=)` | FDTD multiport S-params through the PDK's own openEMS workflow, one excitation per port → touchstone; refuses port layers that collide with the stackup (needs the from-source openEMS build — **verification lane, never inside an optimizer loop**). `sim` = `EmSim` — EVERY solver hyperparameter (band, mesh cell, margin, boundaries, end criterion, excited ports), `EmSim.from_yaml` so the exact setup is committed next to its results (annotated example: `examples/layout/ihp-sg13g2/em_sim.yaml`; a `{tech:, sim:}` combined file loads with both `from_yaml`s) |
| `em.em_to_subckt(touchstone, out, name=, n_poles=, dc_r=)` | touchstone → passivity-enforced vector fit → ngspice `.subckt` with per-port reference pins; `dc_r` anchors f≈0 with the net's DC resistances (FDTD Gauss excitation has ~no DC energy) — after splicing, always compare `.op` against the PEX deck (`scikit-rf`, the `em` extra) |
