...
composes them (P0 deck contract). Live-validated on TSMC-65 Spectre 2026-07-05
...
the PDK map
(`Pdk`/`PdkDevice`/`IHP_SG13G2`/`SKYWATER_SKY130`/`GF180MCU`/`ANALOGGYM_TSMC`/`TSMC_N65`/`get_pdk`/`mos_flavor` — `ANALOGGYM_TSMC`
and `TSMC_N65` are device-**name** maps (the latter is the `translate_ngspice_to_spectre` retarget table for `tsmc-n65`), no foundry content;
