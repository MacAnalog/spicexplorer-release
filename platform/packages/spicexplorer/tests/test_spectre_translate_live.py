"""LIVE translate→compose→Spectre round-trip (opt-in; needs Cadence + TSMC models).

The committed IHP ngspice 5T-OTA
testbench is translated (`deck_spec_from_ngspice`: dialect emitters + TSMC-65
device-model mapping), parameters are injected at the TSMC operating point
...
* ``SPICEXPLORER_TSMC65_MODELS`` — absolute path to the TSMC-65 Spectre model library
...
@pytest.mark.skipif(
    ...
    reason="set SPICEXPLORER_TSMC65_MODELS to the TSMC-65 Spectre model library .scs",
)
def test_live_translated_deck_runs_with_injected_params_and_corner(tmp_path: Path) -> None:
    ...
    # run-time injection through the protocol seam: the TSMC-65 corner + supply
    sim.apply_corner(
        Corner(
            name="tt_27C_1V20",
            model_includes=[
