...
port convention (tails + sub are ports; ideal VCCS tails live in the bench).
    ...
    conda run -n ai_env python pex_sim.py --dut all      # -> out/pex_report.yaml
    python pex_sim.py --dut pam4 --mode RC --skip-pex    # reuse existing PEX
"""
...
KPEX = os.path.expanduser("~/miniconda3/envs/pex/bin/kpex")
KPEX_KLAYOUT = os.path.expanduser("~/local/klayout-py311/klayout-batch.sh")
