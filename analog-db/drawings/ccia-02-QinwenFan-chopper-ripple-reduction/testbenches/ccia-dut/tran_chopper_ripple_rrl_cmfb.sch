...
S {}
E {}
T {ia/tran_chopper_ripple_rrl -- CLOCKED transient at FCHOP with the RRL running} 52.5 -1402.5 0 0 0.4 0.4 {}
...
N 1400 -920 1680 -920 {
lab=voutn}
C {ccia-02-QinwenFan-chopper-ripple-reduction/ccia-dut-chopper-w-positive-feedback-rrl-cmfb.sym} 1015 -725 0 0 {name=XDUT}
...
C {devices/lab_wire.sym} 535 -855 0 0 {name=l42 sig_type=std_logic lab=vcm}
C {devices/gnd.sym} 535 -795 0 0 {name=g43 lab=0}
C {devices/vsource.sym} 805 -825 0 0 {name=Vinp value="pulse(0 \{VIN_DC\} 4m 1n 1n 1 2)" savecurrent=true}
...
C {devices/lab_wire.sym} 1580 -890 0 0 {name=l50 sig_type=std_logic lab=voutp}
C {devices/gnd.sym} 1580 -830 0 0 {name=g51 lab=0}
C {devices/capa.sym} 1680 -890 0 0 {name=CLoadn m=1 value=\{CL\}}
C {devices/lab_wire.sym} 1680 -920 0 0 {name=l52 sig_type=std_logic lab=voutn}
C {devices/gnd.sym} 1680 -860 0 0 {name=g53 lab=0}
C {devices/code.sym} 50 -1315 0 0 {name=PARAMS_BENCH
only_toplevel=true
value="
* TESTBENCH tran_chopper_ripple_rrl_cmfb -- DUT ccia-dut-chopper-w-positive-feedback-rrl-cmfb
* produces: v_ripple_pp, v_ripple0_pp, gain_cl, vos_residual
* recorded baseline: no ia_004 baseline -- the composite has NO CMFB, so the output CM rails once the SC clocks run. ia_002 (no RRL) records v_ripple_pp 0.4022486 V; ia_003 (PF, no RRL) 0.2198775 V.
...
* --- behavioural-ideal CMFB servo --------------------------------------
* Built from primitives (Rm_p/Rm_n + E_cmfb) rather than
* shared/ideal/cmfb-output-ideal-amp-inv.sym, because that block's inner
* ideal-amp-fully-diff uses BARE param names (gm_val/rout_val) which the RRL
* instance already owns -- the servo gain could not be set independently.
*   vcm_ref   commanded output common mode
*   cmfb_rm   CM detector sense resistors (5Meg, not the 1Meg default: this core
*             is a true OTA with high Zout and the detector sits on voutp/voutn)
*   cmfb_gain servo gain A. Plant dvocm/dvb1 ~ 115 V/V, so A=10 gives loop gain
*             ~1150. A=100 SATURATES the servo and the output rails to 1.2 V
*             (measured) -- the amp_030 'stable spurious railed equilibrium'.
.param vcm_ref=0.6097
.param cmfb_rm=5Meg
* TUNED VIA ac_cm_reg (not via transient survival time). Valid points only --
* an AC sweep about a RAILED bias is meaningless, and A=30 rails (vocm 1.193):
*   A    Co     zcm_lf    peaking_db   dc vocm
*   30   any    ~2.98k    ~0.02        1.193  RAILED - numbers are garbage
*   10   10n    27.2      43.2         0.6097
*   10   1u     2.51k      7.05        0.6097
*   10   10u   14.83k      0.23        0.6097
*   3    1u     7.17k      2.42        0.6097  <- picked: passes <=6 dB, stiffest
*   3    10u   22.60k      0.00        0.6097  barely better than open loop
* Open loop (no CMFB) is zcm_lf 28.15k with 0.00 dB peaking.
.param cmfb_gain=3
* servo dominant pole f_p = 1/(2*pi*cmfb_ro*cmfb_co) ~ 16 Hz. Without a pole the
* VCVS is pure gain, the CM loop never rolls off and it oscillates (the clocked
* run died at 78 ns with vocm ringing 0.44<->0.78 V).
* Stability grid on a 200 us clocked run (A x cmfb_co):
*        160p     1n      10n
*   A=10  1.7e-5  3.2e-5  COMPLETES
*   A=3   1.3e-5  COMPLETES COMPLETES
*   A=1   7.0e-5  4.5e-5  COMPLETES
* A=10/10n picked: it completes AND keeps the most loop gain.
.param cmfb_ro=1Meg
.param cmfb_co=1u
...
value="
.control
  save v(voutp) v(voutn) v(vinp) v(vinn) v(vref) v(clk_chop) v(clk_phi_1) i(Vdd)
  ...
  meas tran vavga AVG vd from=3m to=4m
  meas tran vavgb AVG vd from=7m to=8m
  meas tran vdmax MAX vd from=7m to=8m
  meas tran vdmin MIN vd from=7m to=8m
  meas tran vdmax0 MAX vd from=3m to=4m
  meas tran vdmin0 MIN vd from=3m to=4m
  let v_ripple_pp = vdmax - vdmin
  let v_ripple0_pp = vdmax0 - vdmin0
  let gain_cl = (vavgb - vavga)/10m
  let vos_residual = vavga/gain_cl
  print v_ripple_pp v_ripple0_pp gain_cl vos_residual
  * CM DIAGNOSTIC: this core has no CMFB, so the run usually dies of output-CM
  * runaway rather than of anything differential. A vocm that has left VCM (and a
  * vavg* that has collapsed to ~0) means the ripple figures above describe a
  * railed, dead output - not a working amplifier. See CMFB_PLAN.md.
  meas tran vocm_a AVG vocm from=3m to=4m
  meas tran vocm_max MAX vocm
  meas tran vocm_min MIN vocm
  print vocm_a vocm_max vocm_min
  write tran_chopper_ripple_rrl_cmfb.raw
  * --- waveform output ---------------------------------------
  * hardcopy -> SVG, works in batch. plot -> interactive only.
  hardcopy tran_chopper_ripple_rrl_cmfb_vodm.svg vodm
  hardcopy tran_chopper_ripple_rrl_cmfb_cm.svg v(voutp) v(voutn) vocm
  plot vodm
  plot v(voutp) v(voutn) vocm
.endc
"}
