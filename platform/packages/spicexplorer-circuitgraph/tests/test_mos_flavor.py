...
def test_retarget_refuses_a_flavor_the_target_pdk_does_not_have():
    # FOUNDRY-N65's table declares core devices only. Silently emitting `nch_lvt` for a 3.3 V part
...
def test_model_for_stays_exact_so_a_declared_threshold_would_win():
    ...
    assert FOUNDRY_N65.resolve_model(mos, nmos, "lvt")[0] == "nch_lvt"
    assert FOUNDRY_N65.resolve_model(mos, nmos, "hv") == (None, "")
    assert FOUNDRY_N65.resolve_model(mos, nmos, "") == ("nch_lvt", "")  # unflavored: byte-identical
