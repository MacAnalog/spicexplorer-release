...
tsmc-n65 tables come from a licensed kit). The canonical store lives **out-of-repo** at
...
python tools/regen_gmid_luts.py --open-only     # skip the licensed Spectre (tsmc-n65) lane
```

- **Open PDKs** (sky130 / ihp-sg13g2 / gf180mcu) → native ngspice + `$PDK_ROOT` (per-L parallel).
- **tsmc-n65** → headless Spectre via the virtuoso-bridge (licensed kit; needs the operator wrapper
  `$SPICEXPLORER_TSMC_N65_MODEL_ROOT/<corners.lib_file>` and the bridge `local.env`).
...
| tsmc-n65 | `nch_lvt`, `pch_lvt` (SVT `nch`/`pch` + HVT `nch_hvt`/`pch_hvt` once the wrapper exposes them) | Spectre |
